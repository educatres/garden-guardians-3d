...
function buildWorld(){
  ...
  for(let i=0;i<18;i++){const t=cylinder(.08,.12,1+Math.random()*1.3,0x6b4c2e);const crown=sphere(.5+Math.random()*.35,0x3f8f46);t.position.set(-7+Math.random()*30,.5,-8+Math.random()*16);crown.position.copy(t.position).add(new THREE.Vector3(0,.8,0));scene.add(t,crown)}
}
...
function pointerXY(e){const p=e.touches?.[0]||e;pointer.x=p.clientX/innerWidth*2-1;pointer.y=-(p.clientY/innerHeight)*2+1}
canvas.addEventListener('pointerdown',e=>{if(paused||gameOver)return;pointerXY(e);raycaster.setFromCamera(pointer,camera);
...
function loop(now){requestAnimationFrame(loop);const dt=Math.min((now-last)/1000,.05);last=now;if(!paused&&!gameOver){updatePlants(dt);updateProjectiles(dt);updateZombies(dt);updateSuns(dt);updateWaves(dt)}renderer.render(scene,camera)}
requestAnimationFrame(loop);setTimeout(()=>document.querySelector('#loading').classList.add('done'),500);
